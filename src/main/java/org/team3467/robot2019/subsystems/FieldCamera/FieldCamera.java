...
public class FieldCamera extends Subsystem
{
     ...
     * 
     * UsbCamera camera1 = CameraServer.getInstance().startAutomaticCapture(0); camera1.setResolution(320, 240); camera1.setFPS(15); UsbCamera camera2 =
     * CameraServer.getInstance().startAutomaticCapture(1); camera2.setResolution(320, 240); camera2.setFPS(20);
     * 
     * CvSink cvSink = CameraServer.getInstance().getVideo(camera1); CvSource outputStream = CameraServer.getInstance().putVideo("Switcher", 320, 240);
     * 
     * Mat image = new Mat();
     * 
     * while(!Thread.interrupted()) {
     * 
     * // TO-DO: Need OI method to get camera switch // if (CommandBase.oi.getGamepad().getRawButton(9)) { // allowCam1 = !allowCam1; // }
     * 
     * if (allowCam1){ cvSink.setSource(camera1); } else{ cvSink.setSource(camera2); }
     * 
     * cvSink.grabFrame(image); outputStream.putFrame(image); }
     * 
     * });
     * 
     * t.start(); }
